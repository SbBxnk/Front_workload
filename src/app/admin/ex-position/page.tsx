...
import { useExPositionList } from './_partial/useExPositionList'
import { useExPositionMutations } from './_partial/useExPositionMutations'
import CreateExPositionModal from './_partial/CreateExPositionModal'
import EditExPositionModal from './_partial/EditExPositionModal'
import DeleteExPositionModal from './_partial/DeleteExPositionModal'
...
export default function ExPositionPage() {
  ...
  const list = useExPositionList()
  const { createExPosition, updateExPosition, deleteExPosition } =
    useExPositionMutations()
  const [selected, setSelected] = useState<ExPosition | null>(null)

  useEffect(() => {
    setBreadcrumbs([{ text: 'ตำแหน่งบริหาร', path: '/admin/ex-position' }])
  }, [setBreadcrumbs])
  ...
  return (
    <div className="rounded-md bg-white p-4 shadow transition-all duration-300 ease-in-out dark:bg-zinc-900 dark:text-gray-400">
      ...
      <CreateExPositionModal onSubmit={(name) => createExPosition.mutate(name)} />
      <EditExPositionModal
        exPosition={selected}
        onSubmit={(id, name) => updateExPosition.mutate({ id, name })}
      />
      <DeleteExPositionModal
        exPosition={selected}
        onConfirm={(id) => deleteExPosition.mutate(id)}
      />
    </div>
  )
}
