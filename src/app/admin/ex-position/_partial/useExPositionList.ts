...
import type { ExpositionSearchParams } from '@/Types'
import type { SortOrder, SortState } from '@/components/Table'

const DEFAULT_PARAMS: ExpositionSearchParams = {
  search: '',
  page: 1,
  limit: 10,
  sort: '',
...
function readParamsFromUrl(): ExpositionSearchParams {
...
function writeParamsToUrl(params: ExpositionSearchParams) {
...
/**
 * จัดการ state ของตารางตำแหน่งบริหาร (ค้นหา/หน้า/เรียงลำดับ) + ดึงข้อมูลผ่าน React Query
 */
export function useExPositionList() {
  const [params, setParams] = useState<ExpositionSearchParams>(DEFAULT_PARAMS)
  ...
  const query = useQuery({
    queryKey: ['exPositions', params],
    queryFn: () => ExpositionServices.getAllExpositions(params),
  })
  ...
  return {
    exPositions: query.data?.payload ?? [],
