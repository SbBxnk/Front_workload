...
const DEFAULT_PARAMS: Required<CourseSearchParams> = {
  search: '',
  page: 1,
  limit: 10,
  sort: '',
...
function readParamsFromUrl(): Required<CourseSearchParams> {
...
function writeParamsToUrl(params: Required<CourseSearchParams>) {
...
/**
 * จัดการ state ของตารางหลักสูตร (ค้นหา/หน้า/เรียงลำดับ) + ดึงข้อมูลผ่าน React Query
 * คืนค่าให้ page นำไปแสดงผลได้ทันที โดยไม่ต้องรู้รายละเอียดการ fetch
 */
export function useCourseList() {
  const [params, setParams] =
    useState<Required<CourseSearchParams>>(DEFAULT_PARAMS)
  ...
  const query = useQuery({
    ...
    queryFn: () => CourseServices.getAllCourses(params),
  })
