...
import { useCourseList } from './_partial/useCourseList'
import { useCourseMutations } from './_partial/useCourseMutations'
import CreateCourseModal from './_partial/CreateCourseModal'
import EditCourseModal from './_partial/EditCourseModal'
import DeleteCourseModal from './_partial/DeleteCourseModal'
...
export default function CoursePage() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ text: 'หลักสูตร', path: '/admin/course' }])
  }, [setBreadcrumbs])
  ...
  return (
    <div className="rounded-md bg-white p-4 shadow transition-all duration-300 ease-in-out dark:bg-zinc-900 dark:text-gray-400">
      ...
      <CreateCourseModal onSubmit={(data) => createCourse.mutate(data)} />
      <EditCourseModal
        course={selected}
        onSubmit={(id, data) => updateCourse.mutate({ id, data })}
      />
      <DeleteCourseModal
        course={selected}
        onConfirm={(id, name) => deleteCourse.mutate({ id, name })}
      />
    </div>
  )
}
