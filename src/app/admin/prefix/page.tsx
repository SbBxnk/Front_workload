...
import { usePrefixList } from './_partial/usePrefixList'
import { usePrefixMutations } from './_partial/usePrefixMutations'
import CreatePrefixModal from './_partial/CreatePrefixModal'
import EditPrefixModal from './_partial/EditPrefixModal'
import DeletePrefixModal from './_partial/DeletePrefixModal'
...
export default function PrefixPage() {
  ...
  const list = usePrefixList()
  const { createPrefix, updatePrefix, deletePrefix } = usePrefixMutations()
  const [selected, setSelected] = useState<Prefix | null>(null)

  useEffect(() => {
    setBreadcrumbs([{ text: 'คำนำหน้า', path: '/admin/prefix' }])
  }, [setBreadcrumbs])
  ...
  return (
    <div className="rounded-md bg-white p-4 shadow transition-all duration-300 ease-in-out dark:bg-zinc-900 dark:text-gray-400">
      ...
      <CreatePrefixModal onSubmit={(name) => createPrefix.mutate(name)} />
      <EditPrefixModal
        prefix={selected}
        onSubmit={(id, name) => updatePrefix.mutate({ id, prefixName: name })}
      />
      <DeletePrefixModal
        prefix={selected}
        onConfirm={(id) => deletePrefix.mutate(id)}
      />
    </div>
  )
}
