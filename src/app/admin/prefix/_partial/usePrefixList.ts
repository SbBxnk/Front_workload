...
import type { PrefixSearchParams } from '@/Types'
import type { SortOrder, SortState } from '@/components/Table'

const DEFAULT_PARAMS: PrefixSearchParams = {
  search: '',
  page: 1,
  limit: 10,
  sort: '',
...
function readParamsFromUrl(): PrefixSearchParams {
...
function writeParamsToUrl(params: PrefixSearchParams) {
...
/**
 * จัดการ state ของตารางคำนำหน้า (ค้นหา/หน้า/เรียงลำดับ) + ดึงข้อมูลผ่าน React Query
 * คืนค่าให้ page นำไปแสดงผลได้ทันที โดยไม่ต้องรู้รายละเอียดการ fetch
 */
export function usePrefixList() {
  const [params, setParams] = useState<PrefixSearchParams>(DEFAULT_PARAMS)
  ...
  const query = useQuery({
    queryKey: ['prefixes', params],
    queryFn: () => PrefixServices.getAllPrefixes(params),
  })
