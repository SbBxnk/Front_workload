...
interface EditPrefixModalProps {
  prefix: Prefix | null
  onSubmit: (prefixId: number, prefixName: string) => void
}

// daisyui modal เปิด/ปิดด้วย checkbox id="modal-edit"
export default function EditPrefixModal({ prefix, onSubmit }: EditPrefixModalProps) {
  const [editName, setEditName] = useState('')

  // sync ค่าเมื่อเลือกคำนำหน้าที่จะแก้ไข
  useEffect(() => {
    setEditName(prefix?.prefix_name ?? '')
  }, [prefix])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    onSubmit(prefix.prefix_id, editName)
