...
import { useBranchList } from './_partial/useBranchList'
import { useBranchMutations } from './_partial/useBranchMutations'
import CreateBranchModal from './_partial/CreateBranchModal'
import EditBranchModal from './_partial/EditBranchModal'
import DeleteBranchModal from './_partial/DeleteBranchModal'
...
export default function BranchPage() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ text: 'สาขา', path: '/admin/branch' }])
  }, [setBreadcrumbs])
  ...
  return (
    <div className="rounded-md bg-white p-4 shadow transition-all duration-300 ease-in-out dark:bg-zinc-900 dark:text-gray-400">
      ...
      <CreateBranchModal onSubmit={(name) => createBranch.mutate(name)} />
      <EditBranchModal
        branch={selected}
        onSubmit={(id, name) => updateBranch.mutate({ id, branchName: name })}
      />
      <DeleteBranchModal
        branch={selected}
        onConfirm={(id) => deleteBranch.mutate(id)}
      />
    </div>
  )
}
