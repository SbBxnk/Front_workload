...
import type { BranchSearchParams } from '@/Types'
import type { SortOrder, SortState } from '@/components/Table'

const DEFAULT_PARAMS: BranchSearchParams = {
  search: '',
  page: 1,
  limit: 10,
  sort: '',
...
function readParamsFromUrl(): BranchSearchParams {
...
function writeParamsToUrl(params: BranchSearchParams) {
...
/**
 * จัดการ state ของตารางสาขา (ค้นหา/หน้า/เรียงลำดับ) + ดึงข้อมูลผ่าน React Query
 * คืนค่าให้ page นำไปแสดงผลได้ทันที โดยไม่ต้องรู้รายละเอียดการ fetch
 */
export function useBranchList() {
  const [params, setParams] = useState<BranchSearchParams>(DEFAULT_PARAMS)
  ...
  const query = useQuery({
    ...
    queryFn: () => BranchServices.getAllBranches(params),
  })
