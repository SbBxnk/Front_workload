...
interface EditBranchModalProps {
  branch: Branch | null
  onSubmit: (branchId: number, branchName: string) => void
}

// daisyui modal เปิด/ปิดด้วย checkbox id="modal-edit"
export default function EditBranchModal({ branch, onSubmit }: EditBranchModalProps) {
  const [editName, setEditName] = useState('')

  // sync ค่าเมื่อเลือกสาขาที่จะแก้ไข
  useEffect(() => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    onSubmit(branch.branch_id, editName)
