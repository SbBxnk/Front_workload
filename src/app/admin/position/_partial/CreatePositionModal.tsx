...
interface CreatePositionModalProps {
  onSubmit: (positionName: string, positionShortName: string) => void
}

// daisyui modal เปิด/ปิดด้วย checkbox id="modal-create"
export default function CreatePositionModal({ onSubmit }: CreatePositionModalProps) {
  const [positionName, setPositionName] = useState('')
  const [positionShortName, setPositionShortName] = useState('')

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    onSubmit(positionName, positionShortName)
    setPositionName('')
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="p-4">
              ...
              <input
                ...
                onChange={(e) => setPositionShortName(e.target.value)}
                type="text"
                className="w-full rounded-md border border-gray-300 px-4 py-2 text-sm font-light text-gray-600 transition-all duration-300 ease-in-out focus:border-blue-500 focus:outline-none dark:border-zinc-600 dark:bg-zinc-800 dark:text-gray-400"
