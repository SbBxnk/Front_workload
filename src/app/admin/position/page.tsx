...
import { usePositionList } from './_partial/usePositionList'
import { usePositionMutations } from './_partial/usePositionMutations'
import CreatePositionModal from './_partial/CreatePositionModal'
import EditPositionModal from './_partial/EditPositionModal'
import DeletePositionModal from './_partial/DeletePositionModal'
...
export default function PositionPage() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ text: 'ตำแหน่งวิชาการ', path: '/admin/position' }])
  }, [setBreadcrumbs])
  ...
  return (
    <div className="rounded-md bg-white p-4 shadow transition-all duration-300 ease-in-out dark:bg-zinc-900 dark:text-gray-400">
      ...
      <CreatePositionModal
        onSubmit={(name, shortName) =>
          createPosition.mutate({ positionName: name, positionShortName: shortName })
        }
      />
      <EditPositionModal
        position={selected}
        onSubmit={(id, name, shortName) =>
      ...
      <DeletePositionModal
        position={selected}
        onConfirm={(id) => deletePosition.mutate(id)}
      />
    </div>
  )
}
