...
export default function EditModal({
  ...
  const [editWorkloadGroup, setEditWorkloadGroup] = useState(workload_group_name)
  useEffect(() => {
    setEditWorkloadGroup(workload_group_name)
  }, [workload_group_name])
  ...
  return (
    <div className="relative z-[100]">
      ...
      />
      <div className="modal" role={`modal-edit${workload_group_id}`}>
        <div className="modal-box rounded-md dark:bg-zinc-800 p-0">
          <form
            onSubmit={(e) => {
              e.preventDefault()
              handleEdit(e, workload_group_id, editWorkloadGroup)
            }}
          >
            ...
            <div className="p-4">
              ...
              <input
                ...
                value={editWorkloadGroup}
                onChange={(e) => setEditWorkloadGroup(e.target.value)}
                type="text"
                className="w-full rounded-md border border-gray-300 px-4 py-2 text-sm font-light text-gray-600 "
                placeholder="กรุณากรอกชื่อกลุ่มภาระงาน"
              />
            </div>
