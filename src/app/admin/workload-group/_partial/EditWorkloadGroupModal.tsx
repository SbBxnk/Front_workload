...
interface EditWorkloadGroupModalProps {
  workloadGroup: WorkloadGroup | null
  onSubmit: (id: number, name: string) => void
}

// daisyui modal เปิด/ปิดด้วย checkbox id="modal-edit"
export default function EditWorkloadGroupModal({
  workloadGroup,
  onSubmit,
}: EditWorkloadGroupModalProps) {
  const [name, setName] = useState('')

  // sync ค่าเมื่อเลือกกลุ่มภาระงานที่จะแก้ไข
  useEffect(() => {
    setName(workloadGroup?.workload_group_name ?? '')
  }, [workloadGroup])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    onSubmit(workloadGroup.workload_group_id, name)
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="p-4">
              ...
              <input
                ...
                placeholder="กรุณากรอกกลุ่มภาระงาน"
                required
              />
            </div>
