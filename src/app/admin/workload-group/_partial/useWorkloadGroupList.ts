...
import type { WorkloadGroupSearchParams } from '@/Types'
import type { SortOrder, SortState } from '@/components/Table'

const DEFAULT_PARAMS: WorkloadGroupSearchParams = {
  search: '',
  page: 1,
  limit: 10,
  sort: '',
...
function readParamsFromUrl(): WorkloadGroupSearchParams {
...
function writeParamsToUrl(params: WorkloadGroupSearchParams) {
...
/**
 * จัดการ state ของตารางกลุ่มภาระงาน (ค้นหา/หน้า/เรียงลำดับ) + ดึงข้อมูลผ่าน React Query
 */
export function useWorkloadGroupList() {
  const [params, setParams] = useState<WorkloadGroupSearchParams>(DEFAULT_PARAMS)
  ...
  const query = useQuery({
    queryKey: ['workloadGroups', params],
    queryFn: () => WorkloadGroupServices.getAllWorkloadGroups(params),
  })
  ...
  return {
    workloadGroups: query.data?.payload ?? [],
