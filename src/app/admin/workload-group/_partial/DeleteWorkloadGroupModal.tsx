...
interface DeleteWorkloadGroupModalProps {
  workloadGroup: WorkloadGroup | null
  onConfirm: (id: number) => void
}

// daisyui modal เปิด/ปิดด้วย checkbox id="modal-delete"
export default function DeleteWorkloadGroupModal({
  workloadGroup,
  onConfirm,
}: DeleteWorkloadGroupModalProps) {
  if (!workloadGroup) return null

  const handleConfirm = (e: React.MouseEvent) => {
    e.preventDefault()
    onConfirm(workloadGroup.workload_group_id)
          ...
          <div className="flex items-center border-b border-gray-200 p-4">
            <h3 className="font-regular flex truncate text-start text-2xl text-gray-600 dark:text-gray-400">
              ...
              <span className="truncate font-semibold text-business1 dark:text-blue-500/80">
                {workloadGroup.workload_group_name}
              </span>
            </h3>
          </div>
