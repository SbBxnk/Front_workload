...
import { useWorkloadGroupList } from './_partial/useWorkloadGroupList'
import { useWorkloadGroupMutations } from './_partial/useWorkloadGroupMutations'
import CreateWorkloadGroupModal from './_partial/CreateWorkloadGroupModal'
import EditWorkloadGroupModal from './_partial/EditWorkloadGroupModal'
import DeleteWorkloadGroupModal from './_partial/DeleteWorkloadGroupModal'
...
export default function WorkloadGroupPage() {
  const { setBreadcrumbs } = useUtility()
  const list = useWorkloadGroupList()
  const { createWorkloadGroup, updateWorkloadGroup, deleteWorkloadGroup } =
    useWorkloadGroupMutations()
  const [selected, setSelected] = useState<WorkloadGroup | null>(null)

  useEffect(() => {
    setBreadcrumbs([{ text: 'กลุ่มภาระงาน', path: '/admin/workload-group' }])
  }, [setBreadcrumbs])
  ...
  return (
    <div className="rounded-md bg-white p-4 shadow transition-all duration-300 ease-in-out dark:bg-zinc-900 dark:text-gray-400">
      ...
      <CreateWorkloadGroupModal
        onSubmit={(name) => createWorkloadGroup.mutate(name)}
      />
      <EditWorkloadGroupModal
        workloadGroup={selected}
        onSubmit={(id, name) => updateWorkloadGroup.mutate({ id, name })}
      />
      <DeleteWorkloadGroupModal
        workloadGroup={selected}
        onConfirm={(id) => deleteWorkloadGroup.mutate(id)}
      />
    </div>
  )
}
