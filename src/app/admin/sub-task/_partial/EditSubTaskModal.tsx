...
import type { SubTask, UpdateSubTaskRequest } from '@/Types'
import SelectMainTask from './SelectMainTask'

interface EditSubTaskModalProps {
  subtask: SubTask | null
  onSubmit: (subtaskId: number, data: UpdateSubTaskRequest) => void
}
...
export default function EditSubTaskModal({
  ...
  // sync ค่าเมื่อเลือกภาระงานย่อยที่จะแก้ไข
  useEffect(() => {
    setEditName(subtask?.subtask_name ?? '')
    setEditTaskId(subtask?.task_id ?? 0)
    setSelectedMainTask(subtask ? String(subtask.task_name) : null)
  }, [subtask])
  ...
  const handleOnChangeMainTask = (task_id: number) => setEditTaskId(task_id)

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    onSubmit(subtask.subtask_id, {
      subtask_name: editName,
      task_id: editTaskId,
    })
