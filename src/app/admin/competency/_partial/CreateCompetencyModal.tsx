...
import type { CreateCompetencyRequest } from '@/Types'

interface CreateCompetencyModalProps {
  onSubmit: (data: CreateCompetencyRequest) => void
}

// daisyui modal เปิด/ปิดด้วย checkbox id="modal-create"
export default function CreateCompetencyModal({
  onSubmit,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    onSubmit({ competency_name: name, competency_order: order })
    setName('')
    setOrder(0)
