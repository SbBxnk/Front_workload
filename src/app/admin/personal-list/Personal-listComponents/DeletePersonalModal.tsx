...
interface DeleteModalProps {
    ...
    isLoading: boolean
    handleDeletePost: (e: React.MouseEvent, prefix_name: string, fname: string, lname: string, id: number) => void
}
export default function DeletePersonalModal({currentData, isLoading, handleDeletePost}: DeleteModalProps) {
    return (
        <>
    {!isLoading ? (
        <>
            {currentData.map((item,u_id: number) => (
                !isLoading && (
                    <div key={u_id} className="relative z-[100]">
                        <input type="checkbox" id={`modal-delete${item.u_id}`} className="modal-toggle" />
                        <div className="modal" role={`modal-delete${item.u_id}`}>
                            <div className="modal-box rounded-md dark:bg-zinc-800">
                                ...
                                <div className="flex justify-end gap-4">
                                    <button
                                        onClick={(e) => handleDeletePost(e, item.prefix_name, item.u_fname, item.u_lname, item.u_id)}
