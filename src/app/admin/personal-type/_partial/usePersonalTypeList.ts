...
import type { PersonalTypeSearchParams } from '@/Types'
import type { SortOrder, SortState } from '@/components/Table'

const DEFAULT_PARAMS: PersonalTypeSearchParams = {
  search: '',
  page: 1,
  limit: 10,
  sort: '',
...
function readParamsFromUrl(): PersonalTypeSearchParams {
...
function writeParamsToUrl(params: PersonalTypeSearchParams) {
...
/**
 * จัดการ state ของตารางประเภทบุคลากร (ค้นหา/หน้า/เรียงลำดับ) + ดึงข้อมูลผ่าน React Query
 * คืนค่าให้ page นำไปแสดงผลได้ทันที โดยไม่ต้องรู้รายละเอียดการ fetch
 */
export function usePersonalTypeList() {
  const [params, setParams] = useState<PersonalTypeSearchParams>(DEFAULT_PARAMS)
  ...
  const query = useQuery({
    queryKey: ['personalTypes', params],
    queryFn: () => PersonalTypeServices.getAllPersonalTypes(params),
  })
  ...
  return {
    personalTypes: query.data?.payload ?? [],
