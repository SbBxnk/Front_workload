...
interface EditPersonalTypeModalProps {
  personalType: PersonalType | null
  onSubmit: (typePId: number, typePName: string) => void
}

// daisyui modal เปิด/ปิดด้วย checkbox id="modal-edit"
export default function EditPersonalTypeModal({ personalType, onSubmit }: EditPersonalTypeModalProps) {
  const [editName, setEditName] = useState('')

  // sync ค่าเมื่อเลือกประเภทบุคลากรที่จะแก้ไข
  useEffect(() => {
    setEditName(personalType?.type_p_name ?? '')
  }, [personalType])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    onSubmit(personalType.type_p_id, editName)
