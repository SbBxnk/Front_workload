...
import { usePersonalTypeList } from './_partial/usePersonalTypeList'
import { usePersonalTypeMutations } from './_partial/usePersonalTypeMutations'
import CreatePersonalTypeModal from './_partial/CreatePersonalTypeModal'
import EditPersonalTypeModal from './_partial/EditPersonalTypeModal'
import DeletePersonalTypeModal from './_partial/DeletePersonalTypeModal'
...
export default function PersonalTypePage() {
  const { setBreadcrumbs } = useUtility()
  const list = usePersonalTypeList()
  const { createPersonalType, updatePersonalType, deletePersonalType } =
    usePersonalTypeMutations()
  const [selected, setSelected] = useState<PersonalType | null>(null)

  useEffect(() => {
    setBreadcrumbs([{ text: 'ประเภทบุคลากร', path: '/admin/personal-type' }])
  }, [setBreadcrumbs])
  ...
  return (
    <div className="rounded-md bg-white p-4 shadow transition-all duration-300 ease-in-out dark:bg-zinc-900 dark:text-gray-400">
      ...
      <CreatePersonalTypeModal
        onSubmit={(name) => createPersonalType.mutate(name)}
      />
      <EditPersonalTypeModal
        personalType={selected}
        onSubmit={(id, name) =>
          updatePersonalType.mutate({ id, typePName: name })
        }
      />
      <DeletePersonalTypeModal
        personalType={selected}
        onConfirm={(id) => deletePersonalType.mutate(id)}
      />
    </div>
  )
}
