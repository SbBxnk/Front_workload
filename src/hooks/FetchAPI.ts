...
const useFetchData = <T,>(endpoint: string): FetchResult<T> => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setLoading(true)
        const response = await axios.get<{ data: T }>(`${process.env.NEXT_PUBLIC_API}${endpoint}`, {
          headers: {
        ...
        if (axios.isAxiosError(error)) {
          setError(new Error(error.response?.data?.message || "ไม่สามารถเชื่อมต่อกับข้อมูลได้"))
        } else {
